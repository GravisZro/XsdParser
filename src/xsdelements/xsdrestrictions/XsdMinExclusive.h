...
public:
  void accept(std::shared_ptr<XsdAbstractElementVisitor> xsdAbstractElementVisitor)
    {
        ...
        xsdAbstractElementVisitor->visit(std::shared_ptr<XsdMinExclusive>(this));
    }
  ...
  std::shared_ptr<XsdMinExclusive> clone(StringMap placeHolderAttributes)
    {
        ...
        auto elementCopy = std::make_shared<XsdMinExclusive>(getParser(), placeHolderAttributes, m_visitorFunction);
  ...
  {
        return xsdParseSkeleton(parseData.node, std::static_pointer_cast<XsdAbstractElement>(std::make_shared<XsdMinExclusive>(parseData.parserInstance, XsdAbstractElement::getAttributesMap(parseData.node), parseData.visitorFunction)));
    }
